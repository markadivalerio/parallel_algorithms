...
func Test(t *testing.T) {
	for _, test := range ShortestPathTests {
		...
		// flist := []func(graph.Node, graph.Graph) Shortest{Dijkstra}
		// flist := []func(graph.Node, graph.Graph) Shortest{BellmanFord}
		// flist := []func(graph.Node, graph.Graph) Shortest{DeltaStep}
		...
		for _, f := range flist {
			func() {
				...
				defer func() {
					...
					fmt.Println(test.Name, ";", fname, ";Execution Time In Milli Seconds:", elapsed)
				}()
